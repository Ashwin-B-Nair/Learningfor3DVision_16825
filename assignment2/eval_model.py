    ...
    for step in range(start_iter, max_iter):
        ...
        #     plt.imsave(f'vis/{step}_{args.type}.png', rend)
            ...
            if args.type=='vox':
                ...
                voxel_visualizer(pred_voxels, output_path=f'vis/{args.type}/{step}_{args.type}.gif', threshold=0.1)
                mesh_visualizer(mesh_gt.detach().cpu(), output_path=f'vis/{args.type}/{step}_gt_mesh.gif')
                
            if args.type=='point':
                
                if args.n_points == 1000:
                    plt.imsave(f'vis/{args.type}_full/{step}_gt_img.png', images_gt)
                    pred_point_cloud = predictions[0].detach().cpu()
                    pointcloud_visualizer(pred_point_cloud, output_path=f'vis/{args.type}_full/{step}_{args.type}.gif')
                    mesh_visualizer(mesh_gt.detach().cpu(), output_path=f'vis/{args.type}_full/{step}_gt_mesh.gif')
