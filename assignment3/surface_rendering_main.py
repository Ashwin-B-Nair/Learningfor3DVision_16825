...
def render(
    ...
    imageio.mimsave('images/part_8_complex_scene.gif', [np.uint8(im * 255) for im in all_images],loop=0)
...
def train_points(
    ...
    for epoch in range(0, cfg.training.num_epochs):
        ...
        for idx in t_range:
            ...
            loss = torch.mean(torch.linalg.norm(distances, dim=-1)) # TODO (Q6): Point cloud SDF loss on distances
            point_loss = loss
...
def train_images(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        if (
            ...
            imageio.mimsave(f'images/part_8_SDF_10view.gif', [np.uint8(im * 255) for im in test_images], loop=0)
            ...
            try:
                ...
                imageio.mimsave(f'images/part_8_geometry_10view.gif', [np.uint8(im * 255) for im in test_images], loop=0)
